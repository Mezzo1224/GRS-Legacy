...
function _createBonusmenue_func()
	...
	if gWindow["bonusmenue"] then
		...
		gWindow["bonusmenue"] = dgsCreateWindow ( screenwidth/2-370/2, 120, 370, 424, "Bonusmenue", false )
		dgsSetAlpha(gWindow["bonusmenue"],1)
		gGrid["bonusliste"] = dgsCreateGridList(0.027,0.0637,0.4649,0.9104,true,gWindow["bonusmenue"])
		...
		gButton["buyBonus"] = dgsCreateButton(0.5054,0.8797,0.2216,0.0896,"",true,gWindow["bonusmenue"])
		dgsSetAlpha(gButton["buyBonus"],1)
		gButton["cancelBonus"] = dgsCreateButton(0.7405,0.8797,0.2216,0.0896,"Menü schliessen",true,gWindow["bonusmenue"])
		...
		dgsSetFont(gLabel["Bonusname"],"default-bold-small")
		...
		gLabel["yourPoints"] = dgsCreateLabel(0.5108,0.7382,0.3081,0.0566,"Deine Bonuspunkte:",true,gWindow["bonusmenue"])
		...
		gLabel["bonusPoints"] = dgsCreateLabel(0.5324,0.7854,0.2351,0.059,"0 Punkte",true,gWindow["bonusmenue"])
		...
		addEventHandler("onDgsMouseClickUp", gButton["buyBonus"],
			function()
				triggerServerEvent ( "bonusBuy", getLocalPlayer(), getLocalPlayer(), dgsGridListGetItemText ( gGrid["bonusliste"], dgsGridListGetSelectedItem ( gGrid["bonusliste"] ), 1 ) )
			end
		)
	end
...
function fillBonusList ()
	...
	local row = dgsGridListAddRow(gGrid["bonusliste"])
	dgsGridListSetItemText ( gGrid["bonusliste"], row, gColumn["bonusName"], " Spielekonsole", false, false )
	...
	local row = dgsGridListAddRow(gGrid["bonusliste"])
	dgsGridListSetItemText ( gGrid["bonusliste"], row, gColumn["bonusName"], " Schachspiel", false, false )
