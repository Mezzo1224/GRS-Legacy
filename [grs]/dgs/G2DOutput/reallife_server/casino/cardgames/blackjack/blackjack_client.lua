...
function createBlackJackCard ( card, x, y, w, h, parentElement )
	...
	if card then
		if card > -1 then
			...
			if card == 0 then
				gImage["cardBG"..cardNR] = dgsCreateImage(x,y,w,h,"images/cards/images/back.png",false,parentElement)
				dgsSetAlpha(gImage["cardBG"..cardNR],1)
			elseif not ( getCardType ( card ) == 10 or getCardType ( card ) == 11 or getCardType ( card ) == 12 ) then
				gImage["cardBG"..cardNR] = dgsCreateImage(x,y,w,h,"images/cards/images/bg.png",false,parentElement)
				dgsSetAlpha(gImage["cardBG"..cardNR],1)
				...
				gImage["cardDeko1"..cardNR] = dgsCreateImage(0.0373,0.0306,0.1716,0.1179,pfad,true,gImage["cardBG"..cardNR])
				dgsSetAlpha(gImage["cardDeko1"..cardNR],1)
				gImage["cardDeko2"..cardNR] = dgsCreateImage(0.7985,0.8646,0.1866,0.1266,pfad,true,gImage["cardBG"..cardNR])
				dgsSetAlpha(gImage["cardDeko2"..cardNR],1)
				gImage["cardDeko3"..cardNR] = dgsCreateImage(0.291,0.3231,0.4403,0.3231,"images/cards/symbols/"..getFittingSymbol(card)..".png",true,gImage["cardBG"..cardNR])
				dgsSetAlpha(gImage["cardDeko3"..cardNR],1)
				gImage["cardDeko4"..cardNR] = dgsCreateImage(0.0448,0.8515,0.2015,0.1354,"images/cards/symbols/"..getFittingSymbol(card)..".png",true,gImage["cardBG"..cardNR])
				dgsSetAlpha(gImage["cardDeko4"..cardNR],1)
				gImage["cardDeko5"..cardNR] = dgsCreateImage(0.7687,0.0218,0.2015,0.1354,"images/cards/symbols/"..getFittingSymbol(card)..".png",true,gImage["cardBG"..cardNR])
				dgsSetAlpha(gImage["cardDeko5"..cardNR],1)
			else
				gImage["cardBG"..cardNR] = dgsCreateImage(x,y,w,h,"images/cards/images/bg.png",false,parentElement)
				dgsSetAlpha(gImage["cardBG"..cardNR],1)
				local pfad = "images/cards/"..cardColors[getCardColor(card)].."/"..cardIDTypeName[getCardType(card)].."_"..cardColorsShort[getCardColor(card)]..".png"
				gImage["cardDeko1"..cardNR] = dgsCreateImage(0.0694,0.1398,0.8681,0.75,"images/cards/images/"..getFittingPicture(card)..".png",true,gImage["cardBG"..cardNR])
				gImage["cardDeko2"..cardNR] = dgsCreateImage(0.792,0.8475,0.168,0.1243,"images/cards/symbols/"..getFittingSymbol(card)..".png",true,gImage["cardDeko1"..cardNR])
				gImage["cardDeko3"..cardNR] = dgsCreateImage(0.0347,0.0339,0.125,0.0975,pfad,true,gImage["cardBG"..cardNR])
				gImage["cardDeko4"..cardNR] = dgsCreateImage(0.8542,0.9025,0.125,0.0975,pfad,true,gImage["cardBG"..cardNR])
				gImage["cardDeko5"..cardNR] = dgsCreateImage(0.0903,0.1568,0.1458,0.0847,"images/cards/symbols/"..getFittingSymbol(card)..".png",true,gImage["cardBG"..cardNR])
			end
...
function showBlackJackWindow ()
	...
	if isElement ( gWindow["blackJack"] ) then
		dgsSetVisible ( gWindow["blackJack"], true )
	else
		gWindow["blackJack"] = dgsCreateWindow(screenwidth/2-161/2,0,161,194,"Einsatz",false)
		dgsWindowSetMovable ( gWindow["blackJack"], false )
		dgsWindowSetSizable ( gWindow["blackJack"], false )
		dgsSetAlpha(gWindow["blackJack"],1)
		
		gScroll["blackJack"] = dgsCreateScrollBar ( 11, 26, 137, 30, true, false, gWindow["blackJack"] )
		dgsScrollBarSetScrollPosition ( gScroll["blackJack"], 0 )
				
		gButton["placeBlackJackBet"] = dgsCreateButton(76,135,66,40,"Setzen",false,gWindow["blackJack"])
		dgsSetAlpha(gButton["placeBlackJackBet"],1)
		
		gEdit["blackJack"] = dgsCreateEdit(12,85,86,35,"0",false,gWindow["blackJack"])
		dgsSetAlpha(gEdit["blackJack"],1)
		
		gLabel[1] = dgsCreateLabel(106,92,50,14,"$ / Chips",false,gWindow["blackJack"])
		...
		gImage[1] = dgsCreateImage(16,135,37,40,"images/inventory/chip.png",false,gWindow["blackJack"])
		dgsSetAlpha(gImage[1],1)
		
		addEventHandler ( "onDgsElementScroll", gScroll["blackJack"], blackJackScrollBarChanged )
		addEventHandler ( "onDgsTextChange-C", gEdit["blackJack"], blackJackEditChanged )
		addEventHandler ( "onDgsMouseClickUp", gButton["placeBlackJackBet"], blackJackPlaceBet )
	end
end

function blackJackPlaceBet ()

	local amount = math.abs ( math.floor ( tonumber ( dgsGetText ( gEdit["blackJack"] ) ) ) )
	if amount > 0 then
		if amount <= calcPlayerMaxGambleBet ( lp, 1 ) then
			...
			dgsSetVisible ( gWindow["blackJack"], false )
			showCursor ( false )
...
function blackJackScrollBarChanged ()
	...
	local money = math.floor ( vioClientGetElementData ( "casinoChips" ) / 100 * dgsScrollBarGetScrollPosition ( gScroll["blackJack"] ) + 0.5 )
	
	scrollBarChanged = true
	dgsSetText ( gEdit["blackJack"], money )
end
...
function blackJackEditChanged ()
	...
	if scrollBarChanged then
		...
		if curTotalEditChips ~= dgsGetText ( gEdit["blackJack"] ) then
			local money = tonumber ( dgsGetText ( gEdit["blackJack"] ) )
			if money then
				if money <= vioClientGetElementData ( "casinoChips" ) then
					...
					dgsScrollBarSetScrollPosition ( gScroll["blackJack"], money )
				else
					dgsSetText ( gEdit["blackJack"], vioClientGetElementData ( "casinoChips" ) )
				end
			else
				dgsSetText ( gEdit["blackJack"], curTotalEditChips )
			end
		end
	end
end
