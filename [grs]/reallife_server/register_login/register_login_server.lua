...
function regcheck_func ( player )
	...
	if dbExist ( "betakeys", "serial LIKE '"..getPlayerSerial(player).."'") and betasystem == true or betasystem == false then
					...
					if pname ~= "player" then
						if isRegistered ( pname ) then
							...
							if testmode == true then
								...
								elseif string.upper ( clantag ) == "[NOVA" or string.upper ( clantag ) == "[VIO" or string.upper ( clantag ) == "[EXO" or string.upper ( clantag ) == "[XTM" or string.upper ( clantag ) == "[COA" or string.upper ( clantag ) == "[VITA" or string.upper ( clantag ) == "[UTM" or string.upper ( clantag ) == "[UL" then
		...
		triggerClientEvent ( player, "showBetaWindow", getRootElement() )
	end	
end
...
function register_func ( player, passwort, bday, bmon, byear, geschlecht, bcode, email, werber )
    if player == client then
        local pname = getPlayerName ( player )
		...
		--	local email = tostring(email)
            ...
            local result = dbExec ( handler, "INSERT INTO players ( UID,email, Name, Serial, IP, Last_login, Geburtsdatum_Tag, Geburtsdatum_Monat, Geburtsdatum_Jahr, Passwort, Geschlecht, RegisterDatum, LastLogin, permaSuppToken) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)", id, email, pname, getPlayerSerial(player), getPlayerIP ( player ), lastlogin, tonumber ( bday), tonumber ( bmon), tonumber ( byear), passwort, geschlecht, registerdatum, lastLoginInt, token )
            if not result then
				...
				playerToken[pname] = token
            end
            if werber then
                if playerUID[werber] then
            ...
            local result = dbExec ( handler, "INSERT INTO forum_accounts (UID) VALUES (?)", id )
            if not result then
            ...
            if not result then
                outputDebugString ( "[register_func 9] Error executing the query" )
            end



            local result = dbExec ( handler, "INSERT INTO dailylogins (UID) VALUES (?)", id )
            if not result then
                outputDebugString ( "[register_func 10] Error executing the query" )
            end


            if not dbExist ( "coins", "Name LIKE '"..pname.."'") and not dbExist ( "coins", "UID LIKE '"..id.."'") then
                local result = dbExec ( handler, "INSERT INTO coins (UID, Name, Coins) VALUES (?, ?, ?)", id, pname, 0 )
                if not result then
                    outputDebugString ( "[register_func 11] Error executing the query" )
                end
            end
            ...
            vioSetElementData ( player, "MainLevel", 0 )
			vioSetElementData ( player, "pLevel", 0 )
            vioSetElementData ( player, "GWXP", 0 )
            vioSetElementData ( player, "GWLevel", 0 )
            vioSetElementData ( player, "Guthaben", 1 )
            vioSetElementData ( player, "Paket", 0 )
            vioSetElementData ( player, "PremiumData", 0 )
            vioSetElementData ( player, "FraktionsWarns", 0 )
			...
			vioSetElementData ( player, "ticketRating",0 )
			vioSetElementData ( player, "ticketRatings",  0 ) 

			-- // Cooperationsystem
			vioSetElementData ( player, "coopID",0 )
			vioSetElementData ( player, "coopRang",  0 ) 
			getServerCoopDataForClient ( player )
            ...
            local result = dbExec ( handler, "INSERT INTO userdata ( UID,Name,Skinid,Telefonnr,LastLogin) VALUES(?,?,?,?,?)", id, pname, Skinid, Telefonnr, zeit )
            if not result then
                outputDebugString ( "[register_func 8] Error executing the query" )
            else
		...
		if vioGetElementData ( player, "loggedin" ) == 0 then
			...
			if pwresult and pwresult[1] then
				...
				if pwresult == hash ( "sha512", passwort ) then	
					...
					toggleAllControls ( player, true )
					-- // Wenn der Name da ist aber nicht sine UID
					if dbExist ( "coins", "Name LIKE '"..pname.."'") and not dbExist ( "coins", "UID LIKE '"..playerUID[pname].."'") then
						dbExec ( handler, "UPDATE ?? SET ??=? WHERE ??=?", "coins", "UID", playerUID[pname], "Name", pname )
						print("ID Korrekt bei Coin für "..pname.." zu ID "..playerUID[pname])
					end
					if not dbExist ( "coins", "UID LIKE '"..playerUID[pname].."'")  then
						dbExec ( handler, "INSERT INTO coins (UID, Name, Coins) VALUES (?,?, ?)", playerUID[pname], pname, 0 )
						print("ID ADD bei Coin für "..pname.." zu ID "..playerUID[pname])
					end
					
					
					local result = dbPoll ( dbQuery ( handler, "SELECT * from coins WHERE Name = ?", pname ), -1 )
					if result then
						if result[1] then
							local dsatz = result[1]
							vioSetElementData( player, "coins", dsatz["Coins"] )
						end
					end
					...
					if result then
						if result[1] then
							...
							onCooldown[getPlayerName(player)] = false
							vioSetElementData ( player, "lastSocialChange", tonumber ( dsatz["lastSocialChange"] ) )
							vioSetElementData ( player, "lastNumberChange", tonumber ( dsatz["lastNumberChange"] ) )
							vioSetElementData ( player, "lastPremCarGive", tonumber ( dsatz["lastPremCarGive"] ) )
							vioSetElementData ( player, "PremiumCars", tonumber ( dsatz["PremiumCars"] ) )
					...
					--		local tokenresult = dbPoll ( dbQuery ( handler, "SELECT ?? FROM ?? WHERE ??=?", "permaSuppToken","players", "UID", playerUID[pname] ), -1 )
							if  dbExist ( "players", "UID LIKE '"..playerUID[pname].."' AND permaSuppToken LIKE '0'") then
									local token = generateString ( 6 )
									dbExec ( handler, "UPDATE ?? SET ??=? WHERE ??=?", "players", "permaSuppToken", token, "UID", playerUID[pname] )
									outputChatBox("Dein Support Token: "..token, player)
								end
							...
							vioSetElementData ( player, "Guthaben", tonumber ( dsatz["Guthaben"] ) )
							vioSetElementData ( player, "alkatime", tonumber ( dsatz["alkatime"] ) )
							vioSetElementData ( player, "Paket", tonumber ( dsatz["PremiumPaket"] ) )
							vioSetElementData ( player, "PremiumData", tonumber ( dsatz["PremiumData"] ) )
							vioSetElementData ( player, "FraktionsWarns",  tonumber ( dsatz["FraktionsWarns"] ) ) 
							-- // Alle Bewertungen zusammengerechnet
							vioSetElementData ( player, "ticketRating", tonumber ( dsatz["ticketRating"] ) )
							-- // Alle Bewertungen
							vioSetElementData ( player, "ticketRatings",  tonumber ( dsatz["ticketRatings"] ) ) 
							
							-- // Cooperation
							vioSetElementData ( player, "coopID", tonumber ( dsatz["coopID"]) )
							vioSetElementData ( player, "coopRang",  tonumber ( dsatz["coopRang"]) ) 
							getServerCoopDataForClient ( player ) -- // Für die Playlist
							if tonumber ( dsatz["coopID"]) > 0 and cooperation[tonumber ( dsatz["coopID"])] then
								bindKey ( player, "y", "down", "chatbox", "t" )
								cooperation[ tonumber (dsatz["coopID"])].members[pname] = "on"
								local fraktion = tonumber( vioGetElementData ( player, "fraktion") )
								if fraktion > 0 then
									vioSetElementData ( player, "fraktion", 0 )
									vioSetElementData ( player, "rang", 0 )
									newInfobox (player, "Du warst in einer Fraktion und in einem Unternehmen.\nDies ist momentan aber nicht möglich. Du wurdest aus deiner\nFraktion entfernt.", 3)
								end
							end
							
							if not cooperation[tonumber ( dsatz["coopID"])]  and tonumber ( dsatz["coopID"]) ~= 0 then
								vioSetElementData ( player, "coopID", 0 )
								vioSetElementData ( player, "coopRang",  0 ) 
								newInfobox (player, "Dein Unternehmen wurde aufgelöst.\nDu bist nun Zivilist.", 3)
							end
						...
						--	giveBooster ( player, 1, 1)
							...
							dbExec ( handler, "UPDATE ?? SET ??=? WHERE ??=?", "userdata", "LastLogin", zeit, "UID", playerUID[pname] )
							...
							checkPremium ( player )
							triggerClientEvent ( player, "loginHUD", getRootElement() )				
							giveAchievement( player, 1 )
							giveAchievement( player, 2 )
							forumLoad ( player )
							if pname == "Mezzo" or pname == "Yoshi" then
...
--		bindKey (player, "f2", "down",  showfaqmenue,player)
	end
end
...
function datasave ( quitReason, reason )
	...
	if allPrivateCars[pname] then
		for i, v in pairs ( allPrivateCars[pname] ) do
			if isElement ( v ) then
				destroyElement ( v )
			end
		end
		allPrivateCars[pname] = nil
	end
	...
	if vioGetElementData ( source, "loggedin" ) == 1 then
		...
		vioSetElementData  ( source, "SaveX", x )
		vioSetElementData  ( source, "SaveY", y )
		vioSetElementData  ( source, "SaveZ", z )
		vioSetElementData  ( source, "SaveRX", rx )
		vioSetElementData  ( source, "SaveRY", ry )
		vioSetElementData  ( source, "SaveRZ", rz )
		vioSetElementData  ( source, "SaveDim", getElementDimension(source) )
		vioSetElementData  ( source, "SaveInt", getElementInterior(source) )
		vioSetElementData  ( source, "SaveActiv", true )
		saveTimer[source] = setTimer ( vioSetElementData, 300000, 1, source,"SaveActiv",false )
...
function datasave_remote ( player )
	...
	if tonumber ( vioGetElementData ( source, "loggedin" )) == 1 then
		...
		fields = fields..", alkatime = '"..math.floor ( vioGetElementData ( source, "alkatime" ) ).."'"
		fields = fields..", PremiumPaket = '"..vioGetElementData ( source, "Paket" ).."'"
		fields = fields..", PremiumData = '"..vioGetElementData ( source, "PremiumData" ).."'"
		fields = fields..", FraktionsWarns = '"..vioGetElementData ( source, "FraktionsWarns" ).."'"
		fields = fields..", lastSocialChange = '"..vioGetElementData ( source, "lastSocialChange" ).."'"
		fields = fields..", lastNumberChange = '"..vioGetElementData ( source, "lastNumberChange" ).."'"
		fields = fields..", lastPremCarGive = '"..vioGetElementData ( source, "lastPremCarGive" ).."'"
		fields = fields..", PremiumCars = '"..vioGetElementData ( source, "PremiumCars" ).."'"
		...
		dbExec ( handler, "UPDATE ?? SET ??=? WHERE ??=?", "coins", "Coins", vioGetElementData ( player, "coins"), "Name", pname )
...
function saveLevelSystem ( player )
	local pname = getPlayerName ( player )
	dbExec ( handler,"UPDATE ?? SET ??=?,??=?, ??=?, ??=?, ??=? WHERE ??=?", "levelsystem","prestigeLevel", vioGetElementData ( player, "pLevel" ),  "MainXP", vioGetElementData ( player, "MainXP" ), "MainLevel", vioGetElementData ( player, "MainLevel" ), "GWXP", vioGetElementData ( player, "GWXP" ), "GWLevel", vioGetElementData ( player, "GWLevel" ), "UID", playerUID[pname])
end
...
function casinoMoneySave ( player )
	...
	if vioGetElementData ( player, "loggedin" ) == 1 then
		...
		dbExec ( handler,"UPDATE userdata SET ??=?, ??=?, ??=? WHERE UID=?", "Chips", chips, "Geld", money, "Bankgeld", bankMoney, playerUID[name] )
	end
end
...
function getDataLevelSystem ( player )
	...
	if result then
		if result[1] then
			...
			vioSetElementData ( player, "MainXP",  tonumber(dsatz["MainXP"]))
			vioSetElementData ( player, "MainLevel", tonumber(dsatz["MainLevel"]))
			vioSetElementData ( player, "GWXP", tonumber(dsatz["GWXP"]))
			vioSetElementData ( player, "GWLevel", tonumber(dsatz["GWLevel"]))
			vioSetElementData ( player, "pLevel", tonumber(dsatz["prestigeLevel"]))
	end
end
...
function forumLoad ( player )
	if ServerConfig["forum"].enableForumsynchronization == true  then
		...
		if result and result[1] then
			dsatz = result[1]
			if tonumber ( dsatz["ForumID"] ) then
				if tonumber ( dsatz["ForumID"] ) == 0 then
					triggerClientEvent ( player, "showForumRegister", player )
				else
					vioSetElementData ( player, "ForumID", tonumber ( dsatz["ForumID"] ) )
					loadForumPermissions(player)
				end
			else
				triggerClientEvent ( player, "showForumRegister", player )
			end
...
function showfaqmenue ( player )

	if tonumber(getElementData ( player, "loggedin" )) == 1 then

	--		triggerClientEvent ( player, "ShowFAQmenueGui", getRootElement() )
	end
end
addEvent("showfaqmenue", true)
addEventHandler("showfaqmenue", root, showfaqmenue)

function resetPasswordFromLogin (player, token, newPW, newPW_repeat)
	local pname = getPlayerName(player)
	if playerToken[pname] == token then
		if newPW == newPW_repeat then
			local newToken = generateString ( 6 )
			playerToken[pname] = newToken
			local newPW = hash("sha512", hash( "sha512", newPW))
			dbExec ( handler, "UPDATE ?? SET ??=?, ??=? WHERE ??=?", "players", "permaSuppToken", newToken, "Passwort", newPW,  "UID", playerUID[pname] )
			triggerClientEvent ( player, "infobox_start", getRootElement(), "Passwort geändert.\nSchlisse das Fenster nun.", 5000, 255, 0, 0 )
		else
			triggerClientEvent ( player, "infobox_start", getRootElement(), "Passwörter stimmen nicht überein.", 5000, 255, 0, 0 )
		end
	else
		triggerClientEvent ( player, "infobox_start", getRootElement(), "Token stimmt nicht", 5000, 255, 0, 0 )
	end
end
addEvent("resetPasswordFromLogin", true)
addEventHandler("resetPasswordFromLogin", root, resetPasswordFromLogin)