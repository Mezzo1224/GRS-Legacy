...
function isFounderOfGang ( player, gangFounderID )
	...
	if isElement ( player ) then
		...
		local result = dbPoll ( dbQuery ( handler, "SELECT ?? FROM ?? WHERE ??=? AND ??=? AND ??=?", "Founder", "gang_members", "UID", playerUID[player], "Gang", gangFounderID, "Founder", "1" ), -1 )
...
function getPlayerGangRank ( player )
	...
	if player then
		...
		local result = dbPoll ( dbQuery ( handler, "SELECT ?? FROM ?? WHERE ??=?", "Rang", "gang_members", "UID", playerUID[getPlayerName(player)] ), -1 )
		if result and result[1] then
...
function createNewGang ( name, leaderSkin, houseID )
	...
	dbExec ( handler, "INSERT INTO ?? ( ??, ??, ?? ) VALUES (?,?,?)", "gang_basic", "Name", name, "LeaderMSG", 'Gang wurde gegruendet.', "HausID", houseID )
	loadGangData ( houseID )
end

function getGangFromName ( name )
	local result = dbPoll ( dbQuery ( handler, "SELECT ?? FROM ?? WHERE ??=?", "HausID", "gang_basic", "Name", name ), -1 )
	if result and result[1] then
