...
loehne_payday[4][0] = 500
loehne_payday[4][1] = 800
loehne_payday[4][2] = 1100
loehne_payday[4][3] = 1400
loehne_payday[4][4] = 1700
loehne_payday[4][5] = 2000
...
function payday ( player )
	...
	if math.floor ( vioGetElementData ( player, "playingtime" ) / 60 ) == ( vioGetElementData ( player, "playingtime" ) / 60 ) then
		...
		local player_payday = {}
		local playingtime = vioGetElementData ( player, "playingtime" )
		local spielzeit = math.floor ( playingtime / 60 )..":"..( playingtime - math.floor ( playingtime / 60 ) * 60 )
		...
		local allTaxes =  player_payday["Fahrzeugsteuer"]
		if allTaxes > 0 then
			sendMSGToStateFactions(getPlayerName(player).." hat Steuern in Wert von "..allTaxes.."$ gezahlt.", 0, 250, 0)
		end
		factionDepotData["money"][1] = factionDepotData["money"][1] + tonumber(allTaxes)
		triggerClientEvent ( player, "newInfobox", player, "Deine Einnahmen wurden auf dein Konto überwiesen!\nDu kannst Detais am ATM mit einem Rechtsklick einsehen! ", 1, 255, 255, 255, 255, 10, "Payday", 15, 79, 32 )
		local coopID, coopRang = getPlayerCoopData ( player )
		if coopID > 0 then
			player_payday["coop"] = cooperationRanks[coopID]["payday"][coopRang]
		else
			player_payday["coop"] = 0
		end
		if not fraktionsNamen[faction] then
			fname = "Zivilist"
		else
			fname = fraktionsNamen[faction]
		end
		print(fraktionsNamen[faction])
		print(rank)
		print(spielzeit)
		local fullMoney = (player_payday["Gesamt"] + vipMoney + player_payday["coop"])
		local text = 	fname..", Rang: "..rank..", Spielzeit: "..spielzeit.." Stunden \n"
		local text = 	text.."Job: "..player_payday["Lohn"].."$; Boni: "..player_payday["Boni"].."$; Zuschüsse: "..player_payday["Zuschuesse"].."$;"
		local text = 	text.."\n--------------\nKosten:\n--------------\nHandykosten: "..player_payday["Handykosten"].."$;\n"
		local text = 	text.."Club: "..player_payday["Clubkosten"].."$;Fahrzeugsteuer: "..player_payday["Fahrzeugsteuer"].."$; Miete: "..player_payday["Miete"].."$;\n"
		local text = 	text.."Zinsen: "..player_payday["Zinsen"].." $\nEinnahmen durch Ganggebiete: "..player_payday["Gangarea"].."$\n\n"
		local text = 	text.."Einnahmen durch deine Firma/Gang: "..player_payday["coop"].."$\nDurch deinen VIP Status hast du zusätzlich "..vipMoney.."$ erhalten.\n"
		print (text)
		
		dbExec ( handler, "INSERT INTO ?? (??,??,??,??) VALUE (?,?,?,?)", "userdata_payday", "UID", "text", "income", "date", playerUID[getPlayerName(player)], text, fullMoney, timesamp )
		triggerClientEvent ( player, "loadPaydaylistClient", getRootElement(), text,  fullMoney, timesamp)
		if isHalloween then
...
function loadPaydayList ( player )
	local result = dbQuery ( handler, "SELECT * FROM ?? WHERE ??=?", "userdata_payday", "UID",  playerUID[getPlayerName(player)] )
    if result then
        local rows, numrows = dbPoll(result, -1)
        if(numrows > 0) then
			for k, rows in ipairs(rows) do
				
				local text = rows.text
				local income = tonumber(rows.income)
				local date = tonumber(rows.date)
				triggerClientEvent ( player, "loadPaydaylistClient", getRootElement(), text, income, date)
            end
		end
    end
end
