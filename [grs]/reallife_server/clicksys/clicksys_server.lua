guiObjectModels = { [2942]=true, [2190]=true, [2754]=true, [3016]=true }
		...
		if clickedElement then
			...
			if getElementData( clickedElement, "bank_ped" ) == true then
				if button == "left" then
					...
					triggerClientEvent ( source, "showPaydayUI", getRootElement() )
				end
		...
		if clickedElement and not secondClickTypes[getElementType(clickedElement)] then
			...
			if getDistanceBetweenPoints3D ( x1, y1, z1, x2, y2, z2 ) <= 10 then
				...
				if guiObjectModels[model] then
					...
					elseif model == 3016 then
							swtClickBox ( clickedElement, button, source )
						end
		...
		if clickedElement then
			if secondClickTypes[getElementType(clickedElement)] then
				...
				elseif clickedElement == source then
					triggerClientEvent ( source, "ShowSelfClickMenue", getRootElement(), true )
				...
				elseif getElementType ( clickedElement ) == "vehicle" then
					...
					elseif not gangAreaUnderPreparation or not playerData[getPlayerName(source)] then
						if not factionVehicle[clickedElement] and not cooperationVehicle[clickedElement] then
							triggerClientEvent ( source, "_createCarmenue", getRootElement(), clickedElement )
						elseif factionVehicle[clickedElement] then
							id, faction, rang, level = factionVehicle[clickedElement].id, factionVehicle[clickedElement].faction, factionVehicle[clickedElement].rang, factionVehicle[clickedElement].level
							if faction == getPlayerFaction ( source ) then
								triggerClientEvent ( source, "_createFactionCarmenue", getRootElement(),clickedElement, id, faction, rang, level, isElementFrozen(clickedElement) )
							else
								newInfobox (source, "Du bist nicht in der Fraktion "..fraktionNames[faction]..".", 3)
							end
						elseif cooperationVehicle[clickedElement] then
							local id, coopID, rang = cooperationVehicle[clickedElement].id, cooperationVehicle[clickedElement].coopID,  cooperationVehicle[clickedElement].rang -- // Fahrzeug
							local name, tuninglevel =   cooperation[coopID].name,  cooperation[coopID].coopTuningLevel -- // Cooperation
							local playerCoop = vioGetElementData ( source, "coopID" )
							if playerCoop ==  cooperationVehicle[clickedElement].coopID then
								triggerClientEvent ( source, "_createCoopCarmenue", getRootElement(),clickedElement, id, faction, rang, isElementFrozen(clickedElement) )
							else
								newInfobox (source, "Du bist nicht im Unternehmen "..name..".", 3)
							end
						end
						vioSetElementData ( source, "clickedVehicle", clickedElement )
						showCursor ( source, true )
...
function self_func ( player )
	...
	if not getElementClicked ( player ) then
		triggerClientEvent ( player, "ShowSelfClickMenue", player, false )
