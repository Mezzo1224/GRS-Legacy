...
function showShopUI ()
    if oneTabIsOpen == false then
        ...
        shopUI["coins"] = DGS:dgsCreateLabel(0.02, 0.08, 0.27, 0.05, "Coins: "..tonumber( vioClientGetElementData (  "coins") ), true, shopUI["window"])    
        ...
        intPackageTab ()
        -- // Informationen
        shopUI["infoImage"] = DGS:dgsCreateImage(0.03, 0.04, 0.40, 0.93, "images/self/shop/placeholder.png", true, shopUI["info"])
        -- // Verlauf
        intShopHistory ()
        -- // Paysafecard
        shopUI.pins = {}
        ...
        addEventHandler( "onDgsWindowClose",  shopUI["window"], 
        function()
            if source == shopUI["window"] then
...
function refreshShopPremiumInfo (text)
    DGS:dgsSetText(shopUI["vip"], text)
end
addEvent ( "refreshShopPremiumInfo", true )
addEventHandler ( "refreshShopPremiumInfo", getRootElement(), refreshShopPremiumInfo )
...
function intShopHistory ()
    ...
    DGS:dgsSetProperty(shopUI["shopHistoryList"],"rowHeight",30)
    ...
    triggerServerEvent ( "getShopHistory", getLocalPlayer() )
end

function addHistoryEntry (data)
    local row = DGS:dgsGridListAddRow ( shopUI["shopHistoryList"] )
    DGS:dgsGridListSetItemText ( shopUI["shopHistoryList"], row, shopUI["shopHistoryList_item"], data.item )
    DGS:dgsGridListSetItemText ( shopUI["shopHistoryList"], row, shopUI["shopHistoryList_price"], data.price )
    DGS:dgsGridListSetItemText ( shopUI["shopHistoryList"], row, shopUI["shopHistoryList_currency"], data.currency )
    DGS:dgsGridListSetItemText ( shopUI["shopHistoryList"], row, shopUI["shopHistoryList_date"], data.date )
end
addEvent ( "addHistoryEntry", true )
addEventHandler ( "addHistoryEntry", getRootElement(), addHistoryEntry )
...
function intVIPLevelTabs ()
    ...
    addEventHandler( "onDgsMouseDoubleClick", shopUI["BuyVipLevelMoney"], 
        function(button, state, x, y)
            if source == shopUI["BuyVipLevelMoney"] then
                if button == 'left' and state == 'up' then
                    ...
                    if Selected ~= -1 then 
                        ...
                        triggerServerEvent ( "buyVIPLevel", lp, lp, id, "money")
                    end
                end
            end
        end)

    addEventHandler( "onDgsMouseDoubleClick", shopUI["BuyVipLevelCoins"], 
        function(button, state, x, y)
            if source == shopUI["BuyVipLevelCoins"] then
                if button == 'left' and state == 'up' then
                    local Selected = DGS:dgsGridListGetSelectedItem(shopUI["vipLevelsList"])
                    if Selected ~= -1 then 
                        local id = DGS:dgsGridListGetItemData(shopUI["vipLevelsList"],Selected, 1, "id")
                        triggerServerEvent ( "buyVIPLevel", lp, lp, id, "coins")
                    end
                end
            end
        end)
        ...
        if source == shopUI["vipLevelsList"] then
            if button == 'left' and state == 'up' then
				...
				if Selected ~= -1 then 
					...
					DGS:dgsSetText(shopUI["vipLevelInfo"], desc.."\nDrücke Doppelt unten auf einen Button um dieses Stufe zu kaufen.")
