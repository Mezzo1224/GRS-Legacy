...
function submitBetaKey (betakey)
    ...
    if dbExist ( "betakeys", "betakey LIKE '"..betakey.."'") then
        if dbExist ( "betakeys", "serial LIKE ''") then
            ...
            infobox (player, "Erfolgreich!\nDu wirst in 5 Sekunden vom Server geworfen.", "success", 15)
            setTimer ( kickPlayer, 5000, 1, player, "Server","Betaverifizierung" )
        else
            infobox (player, "Der Key ist bereits vergeben.", "error", 15)
        end
    else
        infobox (player, "Key gibt es nicht.", "error", 15)
    end
end
