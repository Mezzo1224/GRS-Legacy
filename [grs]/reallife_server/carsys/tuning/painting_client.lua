	...
	if veh then
		...
		addEventHandler ( "onClientGUIClick", gButton["vehColorSelectionAccept"],
			function ()
				...
				triggerServerEvent ( "changeVehicleColorDatabase", lp, lp, getPedOccupiedVehicle ( lp ), color )
			end,
		false )
...
function cselect ()
	local coopID, coopRang = getElementData(getLocalPlayer(), "coopID"), getElementData(getLocalPlayer(), "coopRang")
	if coopID > 0 and coopRang >= 3 then
		showColorSelection ()
	else
		infobox_start_func ( "Du bist in keinem Unternehmen oder nicht befugt.", 7500, 125, 0, 0 )
	end
end
