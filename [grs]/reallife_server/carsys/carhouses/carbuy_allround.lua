			...
			if slot > 0 then
				...
				if vioGetElementData ( player, "everyCarBuyableForFree" ) or differenz >= 0 then
					if hasPlayerLicense ( player, tonumber(vehid) ) then
						...
						if not dbExec ( handler, "INSERT INTO ?? (??, ??, ??, ??, ??, ??, ??, ??, ??, ??, ??, ??, ??, ??, ??, ??) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", "vehicles", "UID", "Typ", "Tuning", "Spawnpos_X", "Spawnpos_Y", "Spawnpos_Z", "Spawnrot_X", "Spawnrot_Y", "Spawnrot_Z", "Farbe", "Paintjob", "Benzin", "Slot", "Sportmotor", "Bremse", "Antrieb", playerUID[pname], vehid, Tuning, spawnx, spawny, spawnz, rx, ry, rz, color, Paintjob, '100', slot, '0', '0', antrieb )  then
...
function carbuyForCoop (player, carprice, vehid, spawnx, spawny, spawnz, rx, ry, rz)
	...
	if not camper[vehid] then
		...
		if tonumber ( cooperation[coopID].storedMoney ) >= carprice then
			local result, num_affected_rows, id =  dbPoll ( dbQuery( handler, "INSERT INTO cooperations_vehicles (coopID,model, X,Y, Z,RX, RY, RZ) VALUES(?,?,?,?,?,?,?,?)", coopID, vehid,spawnx, spawny, spawnz, rx,ry,rz), -1)
			local vehicle = createCooperationVehicle (id, coopID, vehid, spawnx,spawny,spawnz, rx,ry,rz,0,100, "|255|255|255|", 0 )
			local carpriceAjust = (carprice / 100) * 19
			local carprice = (carprice - carpriceAjust)
			cooperation[coopID].storedMoney = cooperation[coopID].storedMoney - carprice
			newInfobox(player,"Erfolgreich gekauft!\nDas Fahrzeug hat, durch Steuerabsetzung, "..formNumberToMoneyString ( carprice ).." gekostet.", 4)
			sendMessageToCooperation ( pname.." hat ein(e/n) "..getVehicleNameFromModel(vehid).." für "..formNumberToMoneyString ( carprice ).." gekauft." , coopID )
			warpPedIntoVehicle ( player, vehicle )
			activeCarGhostMode ( player, 10000 )
			triggerClientEvent ( player, "leaveCarhouse", player )		
			setElementPosition ( vehicle, spawnx, spawny, spawnz )
			...
			newInfobox(player,"Du hast nicht genug Geld in der Unternehmenskasse.", 3)
		end
	else
		newInfobox(player,"Du kannst keinen Camper kaufen.", 3)
	end
