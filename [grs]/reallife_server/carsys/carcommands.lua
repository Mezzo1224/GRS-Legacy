...
function respawnPrivVeh ( carslot, pname )
	...
	if isElement ( vehicle ) then
		...
		dbExec ( handler, "UPDATE ?? SET ??=? WHERE ??=? AND ??=?", "vehicles", "Benzin", vioGetElementData(vehicle,"fuelstate"), "UID", playerUID[pname], "Slot", carslot )
...
function respawnAllVehiclesOnLogin ( player )
	local curcars = vioGetElementData ( player, "curcars" )
	local maxcars = vioGetElementData ( player, "maxcars" )
	local pname = getPlayerName ( player )
...
function sellcarto_func ( player, cmd, target, price, pSlot )
	...
	if target and pSlot and tonumber ( pSlot ) then
		...
		local pname = getPlayerName ( player )
		local result = dbPoll ( dbQuery ( handler, "SELECT ?? FROM ?? WHERE ??=? AND ??=?", "AuktionsID", "vehicles", "UID", playerUID[pname], "Slot", pslot ), -1 )
	...
	if getPedOccupiedVehicleSeat ( player ) == 0 then
		...
		if isElement ( veh ) and vioGetElementData ( veh, "owner" ) == getPlayerName ( player ) or vioGetElementData ( player, "adminlvl" ) >= 5 then
			if isTrailerInParkingZone ( veh ) then
				...
				dbExec ( handler, "UPDATE ?? SET ??=?, ??=?, ??=?, ??=?, ??=?, ??=?, ??=?, ??=?, ??=?, ??=? WHERE ??=? AND ??=?", "vehicles", "Spawnpos_X", Spawnpos_X, "Spawnpos_Y", Spawnpos_Y, "Spawnpos_Z", Spawnpos_Z, "Spawnrot_X", Spawnrot_X, "Spawnrot_Y", Spawnrot_Y, "Spawnrot_Z", Spawnrot_Z, "Farbe", color, "Paintjob", Paintjob, "Benzin", Benzin, "Distance", Distance, "UID", playerUID[pname], "Slot", slot ) 
			else
