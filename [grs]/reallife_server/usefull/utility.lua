...
function string.cut (s, val) -- s for string, val for searched value 
	if type ( s ) == "string" then
		print(s.."-"..val)
		local stringNum = string.find(s, val)
		print(stringNum)
		local stringEnd = string.sub(s, tonumber(stringNum))
		print(stringEnd)
		local stringFin = string.gsub(tostring(stringEnd), val, "")
		print(stringFin)
		return tostring(stringFin)
	end
end
...
function generateString ( len )
    ...
    if tonumber(len) then
        math.randomseed ( getTimestamp()+math.random(0,1000) )
