	...
	if getElementModel ( source ) == 1273 or getElementModel ( source ) == 1272 then
		...
		elseif vioGetElementData ( source, "owner" ) ~= "none" then				-- Verkauft
			if not vioGetElementData ( source, "gangHQOf" ) then
				...
				print(tostring(vioGetElementData(source, "cooperationOwner")))
			end
			local x, y, z = getElementPosition ( source )
			vioSetElementData ( player, "housex", x )
			vioSetElementData ( player, "housey", y )
...
function gethouseid (player )
	...
	local distance = getDistanceBetweenPoints3D ( x1, y1, z1, x2, y2, z2 )
	if distance < 5 and isAdminLevel ( player, 3 ) then -- isSupporter / isPSuporter ging hier net.
		outputChatBox("Der ID des Hauses ist: "..hausid.."!", player, 100, 255, 0)
	end
end

addCommandHandler ( "gethouseid", gethouseid  )