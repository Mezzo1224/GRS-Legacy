...
function Cwbbc:isUserInGroup(nUID, nGroupID)
	...
	local result = dbPoll ( dbQuery ( wbbHandler, "SELECT ?? FROM ?? WHERE ?? = ?", "groupID", "wcf1_user_to_group", "userID", nUID ), -1 )

	if dbExistForum ( "wcf1_user_to_group", "groupID LIKE '"..nGroupID.."' AND  userID LIKE '"..nUID.."'") then
			return true
	else
			return false
