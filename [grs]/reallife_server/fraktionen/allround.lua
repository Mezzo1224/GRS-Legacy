...
fraktionAktionBlip = {} -- // Für Aktionenblips
	...
	if ServerConfig["main"].enableChangeFactionCMD == true then
		if vioGetElementData(player, "loggedin") == 1 then
			...
			if not isInCoop(player) then
				...
				outputChatBox("Du bist in einem Unternehmen.", player, 125, 0, 0)
			end
		end
	end
end
...
function giveFactionBan ( player, time, reason)
	...
	if vioGetElementData(player, "loggedin") == 1 then
		if tonumber(time) > 0 then
			...
			zeit = rt.timestamp + tonumber(time)
		else
			zeit = 0
		end
...
function sendMSGForFaction ( msg, faction, r, g, b )
	...
	outputLog (  msg, "factionmsg", faction )
	if not r then
...
function fstate_func(player)
	...
	if isInCoop(player) then
		print("Du benutzt jetzt cstate")
...
function invite_func ( player, cmd, target )
	...
	local rank = getPlayerRank ( player )
	if isInCoop(player) then
		print("Du benutzt jetzt cInvite")
		return cInvite (player, cmd, target)
	end
...
function uninvite_func ( player, cmd, target )
	...
	local rank = getPlayerRank ( player )
	if isInCoop(player) then
		print("Du benutzt jetzt cUninvite")
		return cUninvite (player, cmd, target)
	end
	if faction > 0 and rank >= 4 then
...
function toggleBlipsVisibility (player, cmd, type)
	if vioGetElementData(player, "loggedin") == 1 then
		if type == "aktionen" then
			toggleBlipAktionenVisibility ( player )
			newInfobox (player, "Erfolgreich", 1, 255,255,255, 255)
		else
			newInfobox (player, "Gibt es nicht, benutzt:\n/tb aktionen | ", 3, 255,255,255, 255, 10, "Fehler")
		end
	end
end
addCommandHandler("toggleblip", toggleBlipsVisibility )
addCommandHandler("tb", toggleBlipsVisibility )

function toggleBlipAktionenVisibility ( player )
	newInfobox (player, "Erfolgreich", 1, 255,255,255, 255)
	local blips = getElementsByType("blip")

	for i, var in pairs(blips) do
		if fraktionAktionBlip[var] then
			if isElementVisibleTo(var, player) then
				setElementVisibleTo ( var, player, false )
			else
				setElementVisibleTo ( var, player, true )
			end
		end
	end
end
...
function startFrespawn ( player, faction )
	if not faction then
	...
	if faction then
		...
		frak = vioGetElementData ( player, "fraktion" )
	end
	for i, var in ipairs(factionVehicleList) do
        if tonumber(var.faction) == frak then
            local veh = var.vehicle
			print(var.id)
           	respawnVehicle ( veh )
