...
function loadFraktionVehicles ( player )
    ...
    for i, var in pairs(factionVehicleList) do
        if isElement(var.vehicle) then
            if var.faction == fraktion then
                local vehicle = var.vehicle
                local id, rang, level = var.id, var.rang, var.level
                ...
                triggerClientEvent ( player, "loadFraktionVehicles", getRootElement(), id, vehicle, model, rang, level, x, y, z, xr, yr, zr)
            end
        end
    end
...
addCommandHandler("lv", loadFraktionVehicles)
...
function fraktionsMenu_setCarRang ( vehicle, rang )
    if tonumber(vehicle) then
        -- // vehicle ist in dem Fall ´ID´
        vehicle = findFactionVehicleByID (vehicle)
        carname = getVehicleName(vehicle)
    else
        carname = getVehicleName(vehicle)
    end
    local faction = getPlayerFaction ( client )
    ...
    if rank >= 4 then
        if rang <= 5 then
            local id =  factionVehicle[vehicle].id
            factionVehicle[vehicle].rang = rang
            ...
            triggerClientEvent ( client, "infobox_start", getRootElement(), "Der Rang ist zu hoch.", 5000, 125, 0, 0 )
        end
    else
        triggerClientEvent ( client, "infobox_start", getRootElement(), "Du bist nicht befugt!", 5000, 125, 0, 0 )
    end
end
            ...
            if result then
                ...
                if(numrows > 0) then
                    for k, rows in ipairs(rows) do
						...
						local veh = findFactionVehicleByID(id)
                        destroyElement(veh)
...
function fraktionsMenu_orten ( id )
    ...
    local rank = getPlayerRank ( client )
    if rank >= 0 then
        if isElement(VehicleFinder[client]) then
            local veh = findFactionVehicleByID(id)
...
function loadLog()
	local faction = getPlayerFaction ( client )
	local file = fileOpen(":grs_cache/logs/fraktion"..faction..".log", true)
	if (file) then
		local text = fileRead(file, getScriptEnd(file))
		triggerClientEvent(client, "loadLog", client, text)
		fileClose(file)
    else
        triggerClientEvent(client, "loadLog", client, "Kein Log vorhanden.")
  end    
end
