...
if getElementClicked() == false then
  ...
  --  memberswarns = guiGridListAddColumn(FrakiGui.gridlist[1], "Verwarnungen", 0.2)
	...
	addEventHandler("onClientGUIClick", FrakiGui.button[6], function()
		...
		local vehicle = guiGridListGetItemText(FrakiGui.gridlist[2], guiGridListGetItemData(FrakiGui.gridlist[2]), 2)
		print(vehicleData)
		
		if id then
			local rang = tonumber(guiGetText(FrakiGui.edit[5]))
			triggerServerEvent("fraktionsMenu_setCarRang", getLocalPlayer(), id, rang )
			refreshVehicles ()
		end
    end, false)
...
function loadFraktionVehicles (id, vehicle, model, Rang, Level, SX, SY, SZ, SXR, SYR, SZR)
	...
	guiGridListSetItemData(FrakiGui.gridlist[2],vehiclelist[id], vehiclesid, vehicle )
	guiGridListSetItemText(FrakiGui.gridlist[2], vehiclelist[id], vehiclesid, id, false, false)
	guiGridListSetItemText(FrakiGui.gridlist[2], vehiclelist[id], vehiclesveh, vehicleName, false, false)
