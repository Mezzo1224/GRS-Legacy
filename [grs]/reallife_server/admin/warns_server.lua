...
function getPlayerWarnCount ( name )
	local result = dbPoll ( dbQuery ( handler, "SELECT ?? FROM ?? WHERE ?? ", "UID",  "warns", "UID", playerUID[name], 0 ), -1 )
	local count = 0
		...
		if suspect then
			...
			if extends and tonumber(extends) ~= nil then
				...
				if extends and extends > 0 then -- and extends <= 365 then
					...
					dbExec ( handler, "INSERT INTO ?? ( ??,??,??,??,??) VALUES (?,?,?,?,?)", "warns", "UID", "adminUID", "reason", "time", "date", playerUID[name], playerUID[admin], reason, timesamp + extends*24*60*60 , timesamp )
