...
adminLevels = {
	[0] = "User",
	[1] = "#9A2EFEClanmember",
	[2] = "#01DFD7Ticketbeauftragter",
	[3] = "#04B404Supporter",
	[4] = "#0000FFModerator",
	[5] = "#FF8000Administrator",
	[6] = "#B40404Administrator M.v",
		...
		if isAdminLevel ( player, 6 ) then
			if not getPlayerName ( alterName ) then	
				if playerUID[alterName] then
					...
					local result2 = dbPoll ( dbQuery ( handler, "SELECT ?? FROM ?? WHERE ?? LIKE ?", "Name", "players", "Name", neuerName ), 0 )
	...
	if isAdminLevel ( admin, 2 ) then
		if target then
			...
			if player then
				...
				outputChatBox ( --[["Job: "..job..",]]"  Telefonnr: "..vioGetElementData ( player, "telenr" ), admin, 200, 200, 0 )
	...
	if respawn then
		if player == "none" or ( isElement ( player ) ) and isAdminLevel ( player, 2 ) then
			...
			elseif respawn == "sfpd" then
				startFrespawn ( player, 1 )
			elseif respawn == "terror" then
				startFrespawn ( player, 4 )
			elseif respawn == "mafia" then
				startFrespawn ( player, 2 )
			elseif respawn == "triaden" then
				startFrespawn ( player, 3 )
			elseif respawn == "news" then
				startFrespawn ( player, 5 )
			elseif respawn == "fbi" then
				startFrespawn ( player, 6 )
...
function ochat_func ( player, cmd, ... )
	...
	if isAdminLevel ( player, 1 ) or activeOOC == true then
		...
		triggerClientEvent ( player, "infobox_start", getRootElement(), "\nDer OOC ist\nmomentan deaktiviert.", 5000, 125, 0, 0 )		
	end	
end

function changeOOC (player)
	if isAdminLevel ( player, 6 ) then
		if activeOOC == true then
			activeOOC = false
			outputChatBox ( "OOC (/o) wurde deaktiviert.", getRootElement(), 255, 255, 255, true )	
		else
			activeOOC = true
			outputChatBox ( "OOC (/o) wurde aktiviert.", getRootElement(), 255, 255, 255, true )	
		end
	else
		triggerClientEvent ( player, "infobox_start", getRootElement(), "\n\nDu bist nicht\nauthorisiert!", 5000, 125, 0, 0 )	
	end
		...
		if rank then
			...
			if isAdminLevel ( player, 6 ) then		
				...
				elseif playerUID[target] then
					local frac = tonumber ( dbPoll ( dbQuery ( handler, "SELECT ?? FROM ?? WHERE ??=?", "Fraktion", "userdata", "UID", playerUID[target] ), 0 )[1]["Fraktion"] )
					if frac > 0 then
...
function adKick_func (player, command, kplayer )
	rkick_func ( player, command, kplayer, "AD Abuse" )
end
...
function skydive_func(player,command,tplayer)
 ...
 if ne == true then
	if getElementType(player) == "console" or isAdminLevel ( player, 6 ) and ( not client or client == player ) then
		...
		triggerClientEvent ( player, "infobox_start", getRootElement(), "Diese Funktion wurde ausgeschaltet.", 5000, 255, 0, 0 )		
	end	
end
	...
	if isAdminLevel ( player, 2 ) then
		if radius then	
			...
			if radius <= 50 and radius > 0 then
				...
				outputChatBox ( #spehere_table.." Fahrzeuge respawned!", player, 0, 125, 0 )
        ...
        if target then
            if tonumber(time) then
                if tonumber(time) >= 0 then
                    if stringWithAllParameters ~= nil then
						if tonumber(time) > 0 then
							outputChatBox ( "Du hast von "..getPlayerName(player).." für "..time.." Stunde(n) eine Fraktionssperre erhalten.\nGrund: "..stringWithAllParameters, target, 255, 0, 0 )
							outputChatBox ( "Du hast "..getPlayerName(target).." eine Fraktionssperre für "..time.." Stunde(n) gegeben.", player, 255, 0, 0 )
							giveFactionBan ( target, 3600*time, stringWithAllParameters)
						else
							outputChatBox ( "Du hast von "..getPlayerName(player).."  eine permanente Fraktionssperre erhalten.\nGrund: "..stringWithAllParameters, target, 255, 0, 0 )
							outputChatBox ( "Du hast "..getPlayerName(target).." eine permanente Fraktionssperre gegeben.", player, 255, 0, 0 )
							giveFactionBan ( target, 0, stringWithAllParameters)
						end
                    else
                        triggerClientEvent ( player, "infobox_start", getRootElement(), "Ungültiger Grund, benutze:\n/fban NAME ZEIT GRUND", 5000, 255, 0, 0 )
                    end
                else
                    triggerClientEvent ( player, "infobox_start", getRootElement(), "Ungültige Zeit, benutze:\n/fban NAME ZEIT GRUND", 5000, 255, 0, 0 )
                end
            else
                triggerClientEvent ( player, "infobox_start", getRootElement(), "Ungültige Zeit, benutze:\n/fban NAME ZEIT GRUND", 5000, 255, 0, 0 )
            end
    ...
    if isAdminLevel ( player, 6) then
        ...
        if target then
            if vioGetElementData(target, "loggedin") == 1 then
                outputChatBox ( getPlayerName(player).." hat deine Fraktionssperre entfernt.", target, 255, 0, 0 )
                outputChatBox ( "Du hast die Fraktionssperre von "..getPlayerName(target).." entfernt.", player, 255, 0, 0 )
                dbExec ( handler, "UPDATE ?? SET ??=?, ??=?, ??=? WHERE ??=?", "userdata", "FraktionsBanReason", "", "FraktionsBanDate", "0", "FraktionsBan", 0, "UID", playerUID[getPlayerName(target)] )
            end
        else
            triggerClientEvent ( player, "infobox_start", getRootElement(), "Der Spieler\nexistiert nicht!", 5000, 255, 0, 0 )
        end
						...
						if stringWithAllParameters ~= nil and string.len( stringWithAllParameters ) > 3 then
							...
							elseif playerUID[target] then
								if time > 0 then
									...
									local knastzeit = dbPoll ( dbQuery ( handler, "SELECT ?? FROM ?? WHERE ??=?", "Knastzeit", "userdata", "UID", playerUID[target] ), 0 )[1]["Knastzeit"]
									...
									local prisontimeleftoffline = dbPoll ( dbQuery ( handler, "SELECT ?? FROM ?? WHERE ??=?", "Prison", "userdata", "UID", playerUID[target] ), 0 )[1]["Prison"]
...
addCommandHandler ( "dfban", deleteFactionBanAdmin )
addCommandHandler ( "fban", giveFactionBanAdmin )
...
addCommandHandler ( "adkick", adKick_func )
...
addCommandHandler ( "runban", unban_func )
...
addCommandHandler ( "delacc", function ( player, cmd, target )
	if isAdminLevel ( player, 9 ) then
		if playerUID[target] then
			...
			dbExec ( handler, "DELETE FROM ?? WHERE ?? = ?", "levelsystem", "UID", id )
			dbExec ( handler, "DELETE FROM ?? WHERE ?? = ?", "fraktion_warns", "UID", id )
			dbExec ( handler, "DELETE FROM ?? WHERE ?? = ?", "achievements", "UID", id )
    ...
    if isAdminLevel ( player, 4 ) then
        if dbExist ( "userdata", "Name LIKE '"..target.."'") and dbExist ( "players", "Name LIKE '"..target.."'") then
            if target and not getPlayerFromName(target)  then
                ...
                outputChatBox ( "Du hast den Spieler "..target.." offline aus seiner Fraktion entfernt!", player, 0, 150, 0 )
                offlinemsg ( "Der Admin "..getPlayerName(player).." hat dich aus deiner Fraktion entfernt!", "Server", target )
            else
                triggerClientEvent ( player, "infobox_start", getRootElement(), "Diesen Spieler gibt\nist online!", 5000, 125, 0, 0 )
            end
        else
            triggerClientEvent ( player, "infobox_start", getRootElement(), "Diesen Spieler gibt\nes nicht!", 5000, 125, 0, 0 )
        end
    else
        triggerClientEvent ( player, "infobox_start", getRootElement(), "\nDu bist\nnicht befugt!", 7500, 125, 0, 0 )
    end
end
addCommandHandler ( "on", offuninvite )
	...
	if isAdminLevel ( player, 2 ) then
		if target then
			if playerUID[target] then
				...
				if playerTempToken[target] then
					outputChatBox ( "Temporärer Token: "..playerTempToken[nametarget], player, 200, 200, 0 )
				end
				if playerToken[target] then
					outputChatBox ( "Permanenter Token: "..playerToken[target], player, 200, 200, 0 )
				end
			else
				newInfobox (player, "Diesen Spieler gibt es nicht", 3)
			end
		else
			newInfobox (player, "Benutze /gt [NAME]", 3)
        end
	else
		newInfobox (player, "Du bist nicht befugt.", 3)
    end
end
addCommandHandler ( "gettoken", getToken )
...
function getPosition ( player )
	if isAdminLevel ( player, 2 ) then
		...
		if veh then
			...
			outputConsole("Position (Fahrzeug):")
			outputConsole(x..", "..y..", "..z)
			outputConsole(rx..", "..ry..", "..rz)
		else
			local x,y,z = getElementPosition(player)
