...
function refreshScoreboard ()
	...
	for id, p in ipairs(getElementsByType("player")) do
		...
		if getElementData  ( p, "loggedin" ) == 1 then
			...
			coop =   tonumber ( getElementData(p,"coopID") )
		else
			spielzeit = "N.a"
			faction = 0
			fname = " - Keine -"
...
function drawScoreboard()
		...
		for i = 0+scroll, 12+scroll do
			...
			--	coopid = tonumber ( pl[i].coop )
				...
				if tonumber(pl[i].coop)  then
					if tonumber(pl[i].coop) > 0 then
						pl[i].fname = clientsideCoopData[pl[i].coop].name
						fr,fg,fb =  clientsideCoopData[pl[i].coop].r,clientsideCoopData[pl[i].coop].g,clientsideCoopData[pl[i].coop].b
					end
				end
