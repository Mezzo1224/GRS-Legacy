		...
		if vioGetElementData ( player, "bail" ) == 0 or vioGetElementData ( player, "prison" ) > 0 then
			...
			if bail <= money then
				...
				if isElement ( vioGetElementData ( player, "arrester" ) ) then
					...
					factionDepotData["money"][1] = factionDepotData["money"][1] + tonumber(bail)
					sendMSGToStateFactions(getPlayerName(player).." hat seine Kaution im Wert von "..bail.."$ gezahlt.", 0, 250, 0)
